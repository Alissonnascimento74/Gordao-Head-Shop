...
export default function Header({
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[#e3e6de] bg-white/90 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-3 sm:px-6 lg:px-8">
        ...
        <a href="#topo" onClick={handleSecretClick} className="flex shrink-0 items-center gap-3 cursor-default">
          <span className="relative block h-11 w-11 overflow-hidden rounded-full ring-1 ring-[#c3cec0]">
            <Image src="/logo-gordao.jpg" alt="Gordão HeadShop" fill sizes="44px" className="object-cover" priority />
          </span>
          <span className="hidden font-display text-lg tracking-wide text-[#1f2b23] sm:block">
        ...
        {showAdmin && (
          <Link
            href="/admin"
            className="animate-fadeIn shrink-0 rounded-full border border-[#c3cec0] px-3 py-1 text-[11px] font-medium uppercase tracking-wide text-[#4caf6d] transition hover:border-[#4caf6d] hover:bg-[#4caf6d]/10"
          >
            Painel Admin
          </Link>
        )}
        ...
        <div className="relative ml-2 hidden flex-1 sm:block">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#5f7767]" />
          <input
            type="text"
            value={query}
            onChange={(e) => onQueryChange(e.target.value)}
            placeholder="Buscar tabaco, sedas, piteiras…"
            className="w-full rounded-full border border-[#e3e6de] bg-[#f5f4ef] py-2.5 pl-10 pr-4 text-sm text-[#1f2b23] placeholder:text-[#8ea395] outline-none transition focus:border-[#4caf6d]"
          />
        </div>
        ...
        <div className="flex flex-1 items-center gap-2 sm:hidden">
          <div className="relative w-full">
            ...
            <input
              ...
              onChange={(e) => onQueryChange(e.target.value)}
              placeholder="Buscar…"
              className="w-full rounded-full border border-[#e3e6de] bg-[#f5f4ef] py-2 pl-9 pr-3 text-sm text-[#1f2b23] placeholder:text-[#8ea395] outline-none focus:border-[#4caf6d]"
            />
          </div>
        </div>
        ...
        <button
          ...
          className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-[#e3e6de] bg-[#f5f4ef] text-[#1f2b23] transition hover:border-[#4caf6d] hover:text-[#4caf6d]"
        >
    ...
    <button
      ...
      className={`shrink-0 rounded-full border px-4 py-1.5 text-sm font-medium transition ${
        active
          ...
          : "border-[#e3e6de] bg-transparent text-[#5f7767] hover:border-[#c3cec0] hover:text-[#1f2b23]"
      }`}
    >
      {label}
    </button>
